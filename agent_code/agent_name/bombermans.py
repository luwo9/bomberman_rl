...
class CNNext(BombermanBundle):
    """
    Uses:
    - CNN -> Vector MLP
    - Epsilon greedy with exponential decay
    - Preview of bomb explosions
    - Crate based rewards
    """
    ...
    def __init__(self):
        ...
        # Regression model
        self._transformer = transforms.AllFieldsBombPreview(WIDTH, HEIGHT)
        cnn = networks.SimpleCNN([3,3,3], ["same", "same", "same"], [8, 32, 64, 64], (WIDTH, HEIGHT))
        out_size = np.prod(cnn.output_size)
        mlp = networks.NeuralNetwork(out_size, [1024, 1024], N_ACTIONS)
        self._neural_net = nn.Sequential(cnn, nn.Flatten(), mlp)
        ...
        self._training_memory_use = training_memory.TrainingMemory(1200)

        self._sampler = samplers.RandomSampler()

        sample_step = (3, 128, self._sampler)
        sample_round = (1, 256, self._sampler)
